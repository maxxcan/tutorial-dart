...
Una enumeración es usada para definir valores constantes nombrados. Un tipo enumerado es declarado usando la palabra clave *enum*. 
...
+ El nombre_enum especifica el nombre del tipo enumaración
+ La lista enumerada es una lista de Identificadores separados por comas.

Cada uno de los símbolos en la lista de enumaración es un valor entero, uno mayor que el símbolo que lo precede. Por defecto, el valor del primer elemento es 0. 
...
#+BEGIN_SRC dart 
  ...
  void main(){
    print(Estado.values);
    Estado.values.forEach((v) => print('Valores: $v, índice: ${v.index}'));
    print('corriendo: ${Estado.corriendo}, ${Estado.corriendo.index}');
    print('corriendo indice: ${Estado.values[1]}');
  }
#+END_SRC
...
#+BEGIN_SRC 
Valores: Estado.ninguno, índice: 0
Valores: Estado.corriendo, índice: 1
Valores: Estado.parado, índice: 2
Valores: Estado.pausado, índice: 3
corriendo: Estado.corriendo, 1
corriendo indice: Estado.corriendo
#+END_SRC

* Funciones 

Las funciones son bloques de código legibles, mantenibles y reutilizables. Una función es un conjunto de declaraciones que ejecutan una tarea específica. Las funciones organizan el programa en bloques lógicos de código. Una vez definidos, las funciones pueden ser llamadas para acceder a su código. Esto hace el código reutilizable. Encima, las funciones hacen fácil leer y mantener el código del programa. 

Una declaración de función le dice al compilador sobre el nombre de la función, devueve un tipo, y parámetros. Una definición de función proporciona el verdadero cuerpo de la función 

| Nº | Funciones                | Descripción                                                      |
|----+--------------------------+------------------------------------------------------------------|
|  1 | Definiendo una función   | Especifica qué y cómo una tarea específica será realizada        |
|  2 | Llamando a una función   | Una función debe ser llamada para ser ejecutada                  |
|  3 | Devolviendo una función  | Las funciones pueden devolver un valor                           |
|  4 | Funciones parametrizadas | Los parámetros son mecanismos para pasar valores a las funciones |


** Parámetros opcionales 

Los parámetros opcionales pueden ser usados cuando los arguementos no son obligatoriamente necesarios para la ejecución de una función. Un parámetro puede ser marcado como opcional por añaduría de una marca de interrogación en su nombre. El parámetro opciconal pude ser establecido  como el último argumento de una función. 

Tenemos tres tipo de parámetros opcionales en Dart. 

| Nº | Parámetro                                | Descripción                                                                   |
|----+------------------------------------------+-------------------------------------------------------------------------------|
|  1 | Parámetro opcional posicional            | Para hacer esto se urá corchetes []                                           |
|  2 | Parámetro opcional nombrado              | El nombre el parámetro debe ser especificado. Se usan las llaves para esto {} |
|  3 | Parámetro opcional con valor por defecto | Tienen un valor por defecto pero también se le puede pasar valores explícitos |

*** Ejemplos 

** Funciones recursivas en Dart 

La recursión es una técnica para iterarar sobre una operación donde una función se llama así misma repetidamente hasta llegar a un resultado. La recursión es lo mejor aplicado cuando necesitamos llamar a la misma función repetidamente con diferentes parámetros dentro de un bucle. 
...
Las fuciones Lambda son un mecanismo conciso para representar funciones. Esas funciones también son llamadas como funciones Flecha. 

Sintaxid 

: [tipo_devuelto] nombre_funcion(parametros)=>expresion; 
...
Una *interfaz* define la sintaxis que cualquier entidad debe cumplir. Las interfaces definen un conjunto de métodos disponibles para un objeto. Dart no tiene una sintaxxis para declarar interfaces. La declaración de clases son en sí mismas interfaces en Dart. 

Las *clases* deberían usar la palabra clave *implements* para ser capaces de usar una interfaz. Es obligatorio para implmenentar una clase proporcionar una implementación concreta de todas las funciones de la interfaz implementada. En otras palabras, una clase debe redefinir cada función en la interfaz que desee implementar. 

Syntáxis. Implementando una Interfaz 

: class identificador implements nombre_interfaz

Ejemplo 

En el siguiente programa, vamos a declarar un clase *Printer*. La clase *ConsolePrinter* implementa la declaración de interfaz implícita para la clase *Printer*. La fución *main* crea un objeto de la clase *ConsolePrinter* usando la palabra clave *new*. Este objeto es usado para invocar la función *print_data* definido en la clase *ConsolePrinter*. 
...
#+BEGIN_SRC dart 
  ...
  class Printer {
    void print_data(){
      print("_____________Printing Data____________");
    }
  }

  class ConsolePrinter implements Printer {
    void print_data(){
      print("_____________Printing to Console___________");
    }
  }
...
: _______________Printing to Console___________


** Implementando Múltiples Interfaces 

Una clase puede implementar múltiples interfaces. La interfaces son separadas por una coma. La *sintáxis* es la siguiente - 

: class identificador implements interface-1,interface-2,interface-3 ...


El siguiente *ejemplo* muestra como implementar múltiples interfaces en Dart. 
